...
import {Blockquote, Card, Flex, Heading, Icon, Text, SimpleGrid, Badge} from "@chakra-ui/react";
...
import {useInfiniteBooks} from "@/hooks/useInfinitiBooks";
...
export default function BooksList({category, initialBooks, limit = 12}: Props) {
	const {books, loading, hasMore, observerRef} = useInfiniteBooks(category, initialBooks, limit);
				...
				{books.map((book) => (
					...
					>
						<Card.Header h={"150px"} p={3}>
							<Image
								src={
									book.volumeInfo.imageLinks ? book.volumeInfo.imageLinks.thumbnail : notImageBook
								}
								alt="книга"
								width={100}
								height={150}
						...
						<Card.Body>
							<Flex direction="column" height="100%">
								<Blockquote.Root variant="solid">
									<Blockquote.Content>
										<Heading size={{md: "md", lg: "lg"}}>{book.volumeInfo.title}</Heading>
									</Blockquote.Content>
								</Blockquote.Root>
